...
describe('CompactTaskCard compact layout (GroceryCard style)', () => {
  ...
  it('opens all attribute buttons when hamburger is tapped', () => {
    ...
    expect(screen.getByLabelText('tasks.editLabels')).toBeTruthy();
    expect(screen.getByLabelText('tasks.editAssignee')).toBeTruthy();
    expect(screen.getByLabelText('tasks.editSchedule')).toBeTruthy();
    expect(screen.getByLabelText('tasks.toggleFlag')).toBeTruthy();
    expect(screen.getByLabelText('tasks.deleteTask')).toBeTruthy();
  });

  it('shows label input when label icon is tapped in menu', () => {
    render(<CompactTaskCard task={baseTask as any} />);
    fireEvent.click(screen.getByLabelText('Open Editor'));
    fireEvent.click(screen.getByLabelText('tasks.editLabels'));

    expect(screen.getByLabelText('tasks.labelInputAria')).toBeTruthy();
  });

  it('toggles flag and blocked state on flag click', () => {
    render(<CompactTaskCard task={baseTask as any} />);
    fireEvent.click(screen.getByLabelText('Open Editor'));
    fireEvent.click(screen.getByLabelText('tasks.toggleFlag'));
  ...
  it.skip('clears schedule when repeat chip is clicked in edit mode', () => {
  ...
  it('shows clear labels button when labels exist', () => {
    ...
    expect(screen.getByLabelText('tasks.clearAllLabels')).toBeTruthy();
  });
  ...
  it('shows editable title input when hamburger is opened (Edit Mode)', () => {
    render(<CompactTaskCard task={baseTask as any} />);
    expect(screen.queryByLabelText('tasks.editTaskTitle')).toBeNull();

    fireEvent.click(screen.getByLabelText('Open Editor'));

    const input = screen.getByLabelText('tasks.editTaskTitle') as HTMLInputElement;
